...
module.exports = function() {
  ...
  return function(request, response, next) {
    ...
    response.render = function(src, options, odone) {
      ...
      var rendered = false, willbeend = false;
      if( app.fire('beforerender', {
        ...
        target.router = app;
        target.router_rendered_base = request.parentURL;
        
        if( app.id ) target.setAttribute('data-router-scope', app.id + '');
        else target.setAttribute('data-router-scope', '');
        
        target.setAttribute('data-router-base', target.router_rendered_base);
