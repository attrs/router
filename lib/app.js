...
var globalscope = window.router_globalscope = window.router_globalscope || {};

function Application(id) {
  if( arguments.length && typeof id !== 'string' ) {
    id = id[0] || id;
    if( !id || !id.parentNode ) return console.warn('[@attrs/router] illegal argument: router(node)');
    return (function() {
      while( id ) {
        if( id.router ) {
          var app = id.router;
          var base = id.router_rendered_base;
  ...
  router.on('writestate', function(e) {
    if( debug ) console.debug('[@attrs/router] writestate', e.detail.href);
    ...
    var app = {
      ...
      response: {
        ...
        config: function(key, value) {
          ...
          if( finished ) return console.warn('[@attrs/router] request \'' + request.href + '\' already finished.');
          finished = true;
  ...
  router.href = function(requesthref, body, options) {
    ...
    if( typeof requesthref !== 'string' ) return console.error('[@attrs/router] argument \'href\' must be a string');
